...
impl CanvasModel {
    ...
    pub fn fill_map(&self, old_board: &Vec<Vec<i64>>, column: usize, value: i64) -> Vec<Vec<i64>> {
        let mut new_board = old_board.clone();
        if new_board[0][column] != 0 || column > 6 {
    ...
    pub fn draw_circle(&self, x: u32, y: u32, r: u32, color: &str, outline: &str) {
        self.canvas_context.as_ref().unwrap().save();
