...
pub fn canvasModel() -> Html {
    ...
    let add_piece = Closure::wrap(Box::new(move |event: web_sys::MouseEvent| {
        ...
        let x = event.offset_x() as f64 - rect.get_left();
        for j in 0..7 {
    ...
    pub fn win(winner: i64) {
        ...
        canvas_context.as_ref().unwrap().set_fill_style_color("#111");
        canvas_context.as_ref().unwrap().fill_text(&print_msg, 130.0, 20.0, None);
    ...
    pub fn fill_map(state: &Vec<Vec<i64>>, column: usize, value: i64) -> Vec<Vec<i64>> {
    ...
    pub fn action(column: usize, callback: bool) -> i64 {
        let paused = paused.clone();
        let game_won = game_won.clone();
        if *paused || *game_won {
            return 0;
    ...
    pub fn draw_circle(x: u32, y: u32, r: u32, fill: &str, stroke: &str) {
        let canvas_context = canvas_context.clone();
        canvas_context.as_ref().unwrap().save();
        canvas_context.as_ref().unwrap().set_fill_style_color(&fill);
        canvas_context.as_ref().unwrap().set_stroke_style_color(&stroke);
        canvas_context.as_ref().unwrap().begin_path();
        canvas_context.as_ref().unwrap().arc(x as f64, y as f64, 25.0, 0.0, 2.0 * std::f64::consts::PI, false);
        canvas_context.as_ref().unwrap().fill(FillRule::NonZero);
        canvas_context.as_ref().unwrap().restore();
    }
    ...
    pub fn draw_mask() {
        let canvas_context = canvas_context.clone();
        canvas_context.as_ref().unwrap().save();
        canvas_context.as_ref().unwrap().set_fill_style_color("#00bfff");
        canvas_context.as_ref().unwrap().begin_path();
        for y in 0..6 {
            for x in 0..7 {
                canvas_context.as_ref().unwrap().arc((75 * x + 100) as f64, (75 * y + 50) as f64, 25.0, 0.0, 2.0 * std::f64::consts::PI, false);
    ...
    pub fn clear() {
        let canvas_context = canvas_context.clone();
        canvas_context.as_ref().unwrap().clear_rect(0.0, 0.0, self.canvas.as_ref().unwrap().width() as f64, self.canvas.as_ref().unwrap().height() as f64);
    }
    
    pub fn animate(column: usize, current_turn: i64, to_row: usize, cur_pos: usize, callback: bool) {
        let mut fg_color = "transparent";
        if current_turn >= 1 {
            fg_color = "#ff4136";
        }
    ...
    pub fn on_region(coord: f64, x: f64, radius: f64) -> bool {
        return ((coord - x) * (coord - x) <= radius * radius);
    }
    
    pub fn ai(ai_move_value: i64) {
        let map = map.clone()
    ...
    pub fn check_state(state: &Vec<Vec<i64>>) -> (i64, i64) {
        let mut win_val = 0;
        let mut chain_val = 0;
        let mut right = 0;
        let mut down = 0;
    ...
    pub fn value(ai_move_value: i64, state: &Vec<Vec<i64>>, depth: i64, mut alpha: i64, mut beta: i64) -> (i64, i64) {
    ...
    pub fn choose(choice: &Vec<usize>) -> i64 {
        let index = self.get_random_val(choice.len());
        return choice[index] as i64;
    }
    
    pub fn get_random_val(val: usize) -> usize {
    ...
    pub fn max_state(ai_move_value: i64, state: &Vec<Vec<i64>>, depth: i64, mut alpha: i64, mut beta: i64) -> (i64, i64) {
    ...
    pub fn min_state(ai_move_value: i64, state: &Vec<Vec<i64>>, depth: i64, mut alpha: i64, mut beta: i64) -> (i64, i64) {
        let mut v = 100000000007;
        let mut next_move: i64 = -1;
        let mut move_queue = Vec::new();
